...
{-
-- | TypeZipr
newtype TypeZipr base hole
  = TypeZipr (Zipr TypeTooth base hole)

derive instance newtypeTypeZipr :: Newtype (TypeZipr base rec) _

instance showTypeZipr :: (Show base, Show hole) => Show (TypeZipr base hole) where
  show =
    unwrap
      >>> foldMapZipr case _ of
          (BaseType b) -> show b
          (ArrType1 y1 y2) -> "(" <> y1 <> " -> " <> show y2 <> ")"
          (ArrType2 y1 y2) -> "(" <> show y1 <> " -> " <> y2 <> ")"

-- | TypeExpr
newtype TypeExpr base
  = TypeExpr (Expr TypeTooth base)

derive instance newtypeType :: Newtype (TypeExpr base) _

instance showTypeExpr :: Show base => Show (TypeExpr base) where
  show =
    unwrap
      >>> foldMapZipr
          ( matchType
...
matchType ::
  ...
  TypeTooth isPath isExpr a rec -> a
matchType rec (BaseType b) = rec.base b

matchType rec (ArrType1 y1 y2) = rec.arr y1 y2

matchType rec (ArrType2 y1 y2) = rec.arr y1 y2

-- baseType :: forall base. base -> TypeExpr base
-- baseType b = TypeExpr (Expr (Zipr (BaseType b)))
-- -- | Canonically use ArrType1 as 
-- arrType :: forall a. TypeExpr a -> TypeExpr a -> TypeExpr a
-- arrType (TypeExpr (Expr y1)) (TypeExpr (Expr y2)) = TypeExpr (Expr (Zipr (ArrType1 y1 y2)))
-}
